...
impl VirtualMachine {
    pub fn new(mixer: MixerRef, res: ResourceRef, player: SfxPlayerRef,
        video: VideoRef, sys: SystemRef) -> Self {
    ...
    fn op_copy_video_page(&mut self) {
        ...
        self.video.get_mut().copy_page(src_page_id, dst_page_id, self.vm_variables[VM_VARIABLE_SCROLL_Y]);
    }
    ...
    */
    pub fn check_thread_requests(&mut self) -> Result<()> {
        ...
        for thread_id in 0..VM_NUM_THREADS {
            self.vm_is_channel_active[CUR_STATE][thread_id] = self.vm_is_channel_active[REQUESTED_STATE][thread_id];
    
            let n = self.threads_data[REQUESTED_PC_OFFSET][thread_id];
    
            if n != VM_NO_SETVEC_REQUESTED {
                self.threads_data[PC_OFFSET][thread_id] = if n == 0xFFFE { VM_INACTIVE_THREAD } else { n };
    ...
    fn execute_thread(&mut self) {
        while !self.goto_next_thread {
            ...
            if opcode & 0x80 != 0 {
                ...
                // over all the screen.
                self.video.get_mut().set_data_page(self.res.get().seg_cinematic_idx(), off);
                self.video.get_mut().read_and_draw_polygon(COLOR_BLACK, DEFAULT_ZOOM, &Point::new(x, y));
            ...
            if opcode & 0x40 != 0 {
                ...
                else if opcode & 0x10 != 0 { // 0001 0000 is set
                    x += 0x100;
                }
                ...
                else if opcode & 1 != 0 { // 0000 0001 is set
                ...
                self.video.get_mut().set_data_page(if self.res.get().use_seg_video2() { self.res.get().seg_video2_idx() } else { self.res.get().seg_cinematic_idx() }, off);
                self.video.get_mut().read_and_draw_polygon(0xFF, zoom, &Point::new(x, y));
    ...
    pub fn inp_update_player(&mut self) {
        ...
        if self.sys.get().input().dir_mask & DIR_UP != 0 { // inpJump
            ud = -1;
            m |= 8;
        }
        ...
        if self.sys.get().input().button { // inpButton
            button = 1;
            m |= 0x80;
        }
    ...
    fn inp_handle_special_keys(&mut self) {
        if self.sys.get().input().pause {
            if self.res.get().current_part_id() != GAME_PART1 && self.res.get().current_part_id() != GAME_PART2 {
        ...
        if self.sys.get().input().code {
            ...
            if self.res.get().current_part_id() != GAME_PART_LAST && self.res.get().current_part_id() != GAME_PART_FIRST {
